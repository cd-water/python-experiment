...
from chartGenerate.weather_classify import classify_weather, plot_pie_chart, plot_bar_chart
...
def classify():
    if request.method == 'POST':
        ...
        list1 = csvtolist("./data/seattle-weather.csv")
        weather_classification = classify_weather(list1, x_begin, x_end)
        plt1 = plot_pie_chart(weather_classification, x_begin, x_end)
        fig1 = plt1.gcf()
        fig1.set_size_inches(8, 4)
        plt1.tight_layout()
        fig1.savefig('./chart/classifyPie.png', format='png')
        plt1.show()

        plt2 = plot_bar_chart(weather_classification, x_begin, x_end)
        fig2 = plt2.gcf()
        fig2.set_size_inches(8, 4)
        plt2.tight_layout()
        fig2.savefig('./chart/classifyBar.png', format='png')
        plt2.show()
