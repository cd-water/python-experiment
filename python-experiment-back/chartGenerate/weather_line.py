...
from dataUtils import datatolist, dataclean
...
def weathertoLineChart(filename="seattle-weather.csv", start_time='2022-01-01', end_time='2022-01-31'):
    ...
    start = start_time.split('-')[0] + '年' + str(int(start_time.split('-')[1])) + '月' + str(
        int(start_time.split('-')[2])) + '日'
    end = end_time.split('-')[0] + '年' + str(int(end_time.split('-')[1])) + '月' + str(
        int(end_time.split('-')[2])) + '日'
